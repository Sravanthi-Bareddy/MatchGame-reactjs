...
const TabItem = props => {
  ...
  return (
    <li className={`tabItem-container ${isActive ? 'active-tab' : ''}`}>
      <button
        type="button"
        className="tabItem-button"
        onClick={() => clickTab(tabId)}
      >
        {displayText}
      </button>
    </li>
...
class MatchGame extends Component {
  ...
  getActiveList = category =>
    imagesList.filter(eachItem => eachItem.category === category)
  ...
  render() {
    const {matchImageIndex, score, timer, activeTabId, isTimerRunning} = this.state
